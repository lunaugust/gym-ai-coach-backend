...
export const fitnessProfileSchema = Joi.object({
  ...
  primaryGoals: Joi.array()
    .items(Joi.string().valid("weight_loss", "muscle_gain", "strength", "endurance", "flexibility", "general_fitness"))
    .default([]),
  secondaryGoals: Joi.array()
    .items(Joi.string().valid("weight_loss", "muscle_gain", "strength", "endurance", "flexibility", "general_fitness"))
    .default([]),
...
export const preferencesSchema = Joi.object({
  ...
  availableEquipment: Joi.array()
    .items(Joi.string().valid("dumbbells", "barbell", "machines", "bodyweight", "bands", "kettlebell", "other"))
    .default([]),
  gymAccess: Joi.boolean().default(true),
  homeGymSetup: Joi.boolean().default(false),
  preferredTrainingStyles: Joi.array()
    .items(Joi.string().valid("strength", "hypertrophy", "powerlifting", "crossfit", "yoga", "pilates", "mobility", "endurance"))
    .default([]),
...
export const measurementUpdateSchema = measurementCreateSchema.keys({});
